...
#NoTrayIcon ;不显示托盘图标。
#SingleInstance Ignore ;单词 IGNORE 会跳过对话框并让旧实例继续运行. 换句话说, 试图启动已经运行的脚本会被忽略.
SetWorkingDir, %A_ScriptDir% ;改变脚本的当前工作目录。 A_ScriptDir 当前脚本所在目录的绝对路径. 不包含最后的反斜线（根目录同样如此）。  脚本当前工作目录, 这是脚本访问文件的默认路径. 除非是根目录, 否则路径末尾不包含反斜线. 两个示例: C:\ 和 C:\My Documents. 使用 SetWorkingDir 可以改变当前工作目录. 
SetBatchLines, -1 ;设置脚本的执行速度（影响 CPU 使用率）。设置脚本的执行速度（影响 CPU 使用率）。
